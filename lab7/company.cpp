...
namespace coen79_lab7
{
    ...
    bool company::erase(const std::string& product_name) {
        assert(product_name.length() > 0);
        // COMPLETE THE IMPLEMENTATION...
        if(list_search(head_ptr, product_name) == NULL)
            return false;
        node *replace = list_search(head_ptr, product_name);
        replace->setName(head_ptr->getName());
        replace->setLink(head_ptr->getLink());
        node *del = head_ptr;
